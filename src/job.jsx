...
export default function Job({ job, onItemClick }) {
  ...
  return (
    <Card className="job-card d-flex justify-content-center m-3 h-100 border-0" onClick={() => onItemClick(job.id)}>
      <Card.Header className="job-card-header border-0">
        {job.company_logo ? <img className="d-none d-md-block company-logo" height="50" width="50" src={job.company_logo} alt={job.company} />
                          : <h1><span class="badge font-weight-bold text-white company-logo d-flex justify-content-center align-items-center">{companyInitial}</span></h1> }
      </Card.Header>
